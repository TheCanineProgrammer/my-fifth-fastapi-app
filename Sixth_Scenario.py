...
async def assistant(request: ChatRequest):
    ...
    if last_message.type == "image":
        # Construct OpenAI request with image
        user_text = None
        ...
        messages = [
            {"role": "user", "content": [
                {"type": "text", "text": "Describe the main object in this image. just return the answer and only the main object in persian"},
                {"type": "image_url", "image_url": {"url": last_message.content}}
            ]}
        ]
